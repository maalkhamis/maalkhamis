...
data <- read.csv("CoastHF.csv", row.names=1)
head(data)
...
set <- getSelectedAttributes(ImpVar, withTentative = FALSE)#keep tentative
...
mod <-Predictor$new(gbm.fit, data = X, y = Y) #create predictor object. Add your model object name here (GLM, RF, GBM or SVM)
set.seed(123)
...
ice_curves <- lapply(top5, FUN = function(x) {
  cice <- partial(gbm.fit, pred.var = x, center = TRUE, ice = TRUE, which.class="Positive",
...
Categorical <- "Bleeding"
ice_curves1 <- lapply(Categorical, FUN = function(x) {
  ice <- partial(gbm.fit, pred.var =  'Bleeding', ice = TRUE, center = FALSE, which.class="Positive",
                  prob = T)
  ggplot(ice, rug=T, train = data, aes(x=Bleeding, y = yhat, group = Bleeding)) +
...
interact1 <- Interaction$new(mod, feature = "Age")
plot(interact1)+theme_bw()


pdp.obj <-  FeatureEffect$new(mod, feature = c("Gender","Hemoglobin"), method='pdp')
...
shapley <- Shapley$new(mod, x.interest = X[2307,]);shapley$plot()+ theme_bw()
...
Y[2746] #see if observation was negative postive
#negative = 2307
3106, 3578, 2838
